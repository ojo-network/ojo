...
- Ballot Rewards
  ...
  The set of validators that can earn rewards is updated every `SlashWindow` by taking all the active validators at the end of a `SlashWindow` and updating the `ValidatorRewardSet`. Any validators that join in the middle of a `SlashWindow` will not be able to earn ballot rewards until the next `SlashWindow`.
